...
public class ContainerRepair extends Container
{
    ...
    public void updateRepairOutput()
    {
        ...
        else if (var1.getItem() instanceof IUpgradeable && this.inputSlots.getStackInSlot(1) != null && this.inputSlots.getStackInSlot(1).getItem() == Item.quartz) {
        	NBTTagList NBTList = var1.getUpgradeTagList();
            
    	    int upgradeLevel = 0;
    	    
            if(NBTList != null) {
    	        for (int i = 0; i < var1.getUpgradeTagList().tagCount(); i++) {
    	            upgradeLevel = ((NBTTagCompound)NBTList.tagAt(i)).getShort("lvl");
    	        }
            }
        	
            if(upgradeLevel < 15) {
	        	this.stackSizeToBeUsedInRepair = 1;
	        	this.outputSlot.setInventorySlotContents(1, var1);
	            this.maximumCost = 5;
            }
            else {
            	this.outputSlot.setInventorySlotContents(0, (ItemStack)null);
