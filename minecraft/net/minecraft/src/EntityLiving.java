...
public abstract class EntityLiving extends Entity
{
    ...
    protected void damageEntity(DamageSource par1DamageSource, int par2)
    {
        if (!this.isEntityInvulnerable())
        {
            ...
            try {
            	par2 = applyElementarCalculations(par1DamageSource, par2);
            }
    ...
    private int applyElementarCalculations(DamageSource DS, int par2) throws NullPointerException {
    	if(DS instanceof EntityDamageSource)
    	{
    		EntityDamageSource EDS = (EntityDamageSource)DS;
    		if(EDS.getSourceOfDamage() instanceof EntityPlayer) {
    			EntityPlayer entity = (EntityPlayer) EDS.getEntity();
    			int[] attacks = entity.getCombatData()[0];
    			int[] resists = { 0, fireResist, waterResist, earthResist, windResist, lightResist, darkResist, thunderResist, iceResist };
		    	
		    	for(int i=1;i<attacks.length;i++) {
					if(attacks[i] > 0) {
						double mult = (100.0+((double)attacks[i] - (double)resists[i])) / 100.0;
						par2 = (int) Math.round((par2 * mult));
					}
				}
    		}
    		else if(EDS.getSourceOfDamage() instanceof EntityLiving) {
	    		EntityLiving entity = (EntityLiving) EDS.getSourceOfDamage();
	    		int[] attacks = entity.getCombatData()[0];
	    		int[] resists = { 0, fireResist, waterResist, earthResist, windResist, lightResist, darkResist, thunderResist, iceResist };
