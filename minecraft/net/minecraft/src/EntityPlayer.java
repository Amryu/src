...
public abstract class EntityPlayer extends EntityLiving implements ICommandSender
{
    ...
    private void setCombatData() throws NullPointerException {
    	resetCombatAttack();
    	resetCombatDefense();
    	
    	EntityLiving entity = new EntityCow(worldObj);

        int a = inventory.getDamageVsEntity(entity);
        //TODO Elementschwertern Effekte geben
        if (isPotionActive(Potion.damageBoost))
        {
            a += 3 << getActivePotionEffect(Potion.damageBoost).getAmplifier();
        }

        if (isPotionActive(Potion.weakness))
        {
            a -= 2 << getActivePotionEffect(Potion.weakness).getAmplifier();
        }

        int k = 0;

        k = EnchantmentHelper.getEnchantmentModifierLiving(this, entity);
        
        attack = a + k;
        
		try {
			if(inventory.getCurrentItem().getItem() instanceof ItemSword) {
				ItemSword weapon = (ItemSword) inventory.mainInventory[inventory.currentItem].getItem();
	    		
	    		fireAttack += weapon.getToolMaterial().getFireAttack();
	    		waterAttack += weapon.getToolMaterial().getWaterAttack();
	    		earthAttack += weapon.getToolMaterial().getEarthAttack();
	    		windAttack += weapon.getToolMaterial().getWindAttack();
	    		lightAttack += weapon.getToolMaterial().getLightAttack();
	    		darkAttack += weapon.getToolMaterial().getDarkAttack();
	
	    		thunderAttack += weapon.getToolMaterial().getThunderAttack();
	    		iceAttack += weapon.getToolMaterial().getIceAttack();
	    	}
	    	else if (inventory.getCurrentItem().getItem() instanceof ItemTool) {
	    		ItemTool weapon = (ItemTool) inventory.mainInventory[inventory.currentItem].getItem();
	    		
	    		fireAttack += weapon.toolMaterial.getFireAttack();
	    		waterAttack += weapon.toolMaterial.getWaterAttack();
	    		earthAttack += weapon.toolMaterial.getEarthAttack();
	    		windAttack += weapon.toolMaterial.getWindAttack();
	    		lightAttack += weapon.toolMaterial.getLightAttack();
	    		darkAttack += weapon.toolMaterial.getDarkAttack();
	
	    		thunderAttack += weapon.toolMaterial.getThunderAttack();
	    		iceAttack += weapon.toolMaterial.getIceAttack();
	    	}
		} catch (NullPointerException e) {}
    	
    	for(int i = 0; i < 4; i++) {
    		try { 
    			ItemArmor armor = (ItemArmor) inventory.armorItemInSlot(i).getItem();
    		
	    		fireResist += armor.getMaterial().getFireResistance();
	    		waterResist += armor.getMaterial().getWaterResistance();
	    		earthResist += armor.getMaterial().getEarthResistance();
	    		windResist += armor.getMaterial().getWindResistance();
	    		lightResist += armor.getMaterial().getLightResistance();
	    		darkResist += armor.getMaterial().getDarkResistance();
	    		
	    		thunderResist += armor.getMaterial().getThunderResistance();
	    		iceResist += armor.getMaterial().getIceResistance();
    		} catch (NullPointerException e) {}
    	}
    	
    	for(int i = 0; i < 4; i++) {
    		try { 
//	    		ItemEquip equip = (ItemEquip) inventory.equipItemInSlot(i).getItem();
//	    		
//	    		if(equip.getMode() == 0) {
//	        		fireAttack += equip.getFireAttack();
//	        		waterAttack += equip.getWaterAttack();
//	        		earthAttack += equip.getEarthAttack();
//	        		windAttack += equip.getWindAttack();
//	        		lightAttack += equip.getLightAttack();
//	        		darkAttack += equip.getDarkAttack();
//	
//	        		thunderAttack += equip.getThunderAttack();
//	        		iceAttack += equip.getIceAttack();
//	    			
//	    			
//		    		fireResist += equip.getFireResistance();
//		    		waterResist += equip.getWaterResistance();
//		    		earthResist += equip.getEarthResistance();
//		    		windResist += equip.getWindResistance();
//		    		lightResist += equip.getLightResistance();
//		    		darkResist += equip.getDarkResistance();
//		    		
//		    		thunderResist += equip.getThunderResistance();
//		    		iceResist += equip.getIceResistance();
//	    		}
//	    		else {
//	        		fireAttack *= equip.getFireAttack() / 100;
//	        		waterAttack *= equip.getWaterAttack() / 100;
//	        		earthAttack *= equip.getEarthAttack() / 100;
//	        		windAttack *= equip.getWindAttack() / 100;
//	        		lightAttack *= equip.getLightAttack() / 100;
//	        		darkAttack *= equip.getDarkAttack() / 100;
//	
//	        		thunderAttack *= equip.getThunderAttack() / 100;
//	        		iceAttack *= equip.getIceAttack() / 100;
//	    			
//	    			
//		    		fireResist *= equip.getFireResistance() / 100;
//		    		waterResist *= equip.getWaterResistance() / 100;
//		    		earthResist *= equip.getEarthResistance() / 100;
//		    		windResist *= equip.getWindResistance() / 100;
//		    		lightResist *= equip.getLightResistance() / 100;
//		    		darkResist *= equip.getDarkResistance() / 100;
//		    		
//		    		thunderResist *= equip.getThunderResistance() / 100;
//		    		iceResist *= equip.getIceResistance() / 100;
//	    		}
    		} catch (NullPointerException e) {}
    	}
    	
    	defense = getTotalArmorValue() + EnchantmentHelper.getEnchantmentModifierDamage(inventory.armorInventory, null);
    	
    	if(fireResist > 100) fireResist = 100;
    	if(waterResist > 100) waterResist = 100;
    	if(earthResist > 100) earthResist = 100;
    	if(windResist > 100) windResist = 100;
    	if(lightResist > 100) lightResist = 100;
    	if(darkResist > 100) darkResist = 100;

    	if(thunderResist > 100) thunderResist = 100;
    	if(iceResist > 100) iceResist = 100;
	}

	private void resetCombatAttack() {
		attack = 0;
    ...
    private void resetCombatDefense() {
    	defense = 0;
    ...
    /**
     * Displays the crafting GUI for a kitchen.
     */
	public void displayGUIKitchen(int par1, int par2, int par3) {}
    ...
    {
        super.setDead();
        this.inventoryContainer.onWorkbenchGuiClosed(this);

        if (this.openContainer != null)
        {
            this.openContainer.onWorkbenchGuiClosed(this);
        }
    }
